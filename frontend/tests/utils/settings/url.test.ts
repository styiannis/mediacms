...
// Tests for urlConfig following utils-settings suite conventions
// Behaviors:
// 1) Builds admin/signout/changePassword/manage URLs based on user anonymity/admin flags
// 2) Produces embed URL from site.url without trailing slashes
// 3) Builds search URLs for base/query/tag/category
// 4) Builds profile URLs differently for devEnv=true (use user.pages) vs devEnv=false (use site.url + profileId)
// 5) Passes through archive and user URLs unchanged
// 6) Uses site.url correctly regardless of trailing slash
