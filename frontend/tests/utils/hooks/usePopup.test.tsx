...
describe('utils/hooks/usePopup', () => {
    ...
    test('popupContentRef is a mutable ref object with current defaulting to null', () => {
        let ref: any;
        const Comp: React.FC = () => {
            [ref] = usePopup();
            return null;
    ...
    test('does not crash when read without rendering returned components', () => {
        const Comp: React.FC = () => {
            const [_ref, PopupContent, PopupTrigger] = usePopup();
            expect(typeof PopupContent).toBe('function');
            expect(typeof PopupTrigger).toBe('function');
            return null;
        };
